...
def loading(imgid,aux_id,mode):
    ...
    try:
        ...
        image = clip_processor(images=image, return_tensors='np')['pixel_values'].squeeze()
    except:
    ...
    for i in range(min(3, len(aux_img_paths))):
        ...
        aux_img = aux_processor(images=aux_img, return_tensors='np')['pixel_values'].squeeze()
        aux_imgs.append(aux_img)

    # padding
    for i in range(3-len(aux_imgs)):
        aux_imgs.append(np.zeros((3, aux_size, aux_size))) 

    aux_imgs = np.stack(aux_imgs, axis=0)  #[3,3,128,128]
    ...
    for i in range(3-len(rcnn_imgs)):
        rcnn_imgs.append(np.zeros((3, rcnn_size, rcnn_size))) 

    rcnn_imgs = np.stack(rcnn_imgs, axis=0)  #[3,3,64,64]
